...
from utility.federated_learning import fetch_benchmark_and_calculate_price, start_federated_learning
...
async def create_federated_session_v2(
    ...
    federated_manager.log_event(session.id, "Fetching benchmark stats and calculating training price")
    required_data_points = fetch_benchmark_and_calculate_price(session, db)
    federated_manager.log_event(session.id, f"Calculated training price as {required_data_points} data points")
    
    
    # Store the calculated price in the session
    federated_manager.log_event(session.id, f"Storing calculated price in session {session.id}")
    ...
    try:
        ...
        if(session):
            ...
            if decision == 1:
                ...
                message = "Thank you for rejecting the price. The training will not start."
            else:
                raise HTTPException(
                    status_code=400,
